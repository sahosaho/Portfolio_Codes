...
public class Enemy_AI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //transform.position = new Vector3(transform.position.x, Mathf.Clamp(transform.position.y, -1.8f,1.3f), transform.position.z);

        // RaycastHit hit;

        // Debug.DrawRay(transform.position + offset, -transform.up * 1f);
        // if (Physics.Raycast(transform.position + offset, -transform.up, out hit, 1f))   //Down
        // {

        //     if (!isJumpLimitReached && hit.transform.gameObject.CompareTag("Breakable") && !isJum)
        //     {
        ...
        //             enemy_Rigidbody.velocity = Vector3.up * enemy_Jump_Power * Time.deltaTime;
        //             // enemy_Rigidbody.AddRelativeForce(Vector3.up * enemy_Jump_Power * Time.deltaTime,ForceMode.VelocityChange);
        ...
        if (Physics.Raycast(transform.position + offset1, transform.up, out hit1, 1.5f))     //Up
        {
            ...
            if (!isJumpLimitReached && hit1.transform.gameObject.CompareTag("Breakable") && !isJump1)
            {
                ...
                if (isJump1)
                {
                    ...
                    // enemy_Rigidbody.AddRelativeForce(Vector3.up * enemy_Jump_Power * Time.deltaTime,ForceMode.VelocityChange);
                    // flyingParticle1.SetActive(true);
                    isJumped = true;
                    isJump1 = false;
            ...
            {
                //flyingParticle1.SetActive(false);
            }
